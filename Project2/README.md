...
If not already done, please follow the [AWS Tutorial](https://gitlab.ethz.ch/dlad21/aws-tools) to setup your AWS access.
...
have a look at the AWS cost explorer: https://console.aws.amazon.com/cost-management/home?region=us-east-2#/dashboard.
...
For development, feel free to use your additional m6i.xlarge instance (10x cheaper than p2.xlarge) by setting
...
### Alternative: Tensorboard Monitoring

If you prefer, you can also use tensorboard to monitor your trainings.

First, install tensorboard locally. In most cases, it is as easy as `pip install tensorflow==2.2`.

You can download the tensorboard logs from AWS S3 using:

```shell script
bash aws_sync_tensorboards.sh
```

After the download, the script will automatically start a local tensorboard server. You can access tensorboard
