...
def setup_s3_bucket():
    if not os.path.exists("aws_configs/default_s3_bucket.txt"):
        ...
        print("Choose an existing or new name for your bucket (e.g. dlad22-teamXX-ex2 where XX is your team number) "
              "according to the naming rule (https://docs.aws.amazon.com/awscloudtrail/latest/userguide/cloudtrail-s3-bucket-naming-requirements.html).")
...
def setup_team_id():
    if not os.path.exists("aws_configs/team_id.txt"):
        team_id = input("Please enter your DLAD team ID as raw number: ")
        try:
            int(team_id)  # test if conversion is valid
        except ValueError:
            print("Your team ID is not a valid integer.")
            quit()
        assert 0 <= int(team_id) < 100, "Your team ID should be between 0 and 100."
        with open("aws_configs/team_id.txt", "w") as fh:
            fh.write(team_id)
